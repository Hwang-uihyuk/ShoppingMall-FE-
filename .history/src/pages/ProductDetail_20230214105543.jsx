...
console.log(selected)
  ...
  //좋아요등록하기
  const handleLike = () => { axios.post(`http://3.38.35.43:8080/user/cart/${detaildata.id}`,{
    headers : {
      'Content-Type' : 'application/json',
      'Authorization' : window.localStorage.getItem('Login')
  }
...
{/* 내가만든 select */}
          ...
          <Button text='상품구매하기'  />
          <button onClick={handleLike}className='bg-slate-100 border'> 좋아요{detaildata.favorite} </button>
          </span>
        </div>
      </section>
    </>
