      ...
      className='w-70 overflow-hidden cursor-pointer transition-all'>
        <div className='p-5 hover:bg-slate-100'>
          ...
          <div className='mt-2 px-2 text-base column justify-between items-left'>
            <h3 className='font-normal truncate text-md'>{name}</h3>
            <p className='font-semibold text-sm'>{`${price.toLocaleString('to-KR')} 원`}</p>
          </div>
          <p className='flex items-center mb-2 px-2 mt-1  font-semibold text-sm text-pink-700 flex'>
            <img src={'/images/heart.png'} className='w-4 h-4 mr-1' />{favorite}</p>
